...
export async function GET() {
  if (!botEnabled()) {
    return NextResponse.json({
      enabled: false,
      greeting: "Der Bot ist nicht eingerichtet. Dafuer fehlt AI_GATEWAY_API_KEY.",
      suggestions: [],
      conversationId: null,
    });
  }
...
export async function POST(req: Request) {
  if (!botEnabled()) {
    return errorStream("Der Bot ist nicht eingerichtet. Dafuer fehlt AI_GATEWAY_API_KEY.");
  }
