...
describe("streamChat / SSE-Parser", () => {
  ...
  it.each([
    ...
    [{type:"content_block_delta",index:0,delta:{type:"text_delta",text:"Teil"}}],
    [{type:"message_delta",delta:{stop_reason:"max_tokens"}},{type:"message_stop"}],
    [{type:"content_block_start",index:0,content_block:{type:"tool_use",id:"x",name:"write"}},{type:"message_stop"}],
    [{type:"content_block_start",index:0,content_block:{type:"tool_use",id:"x",name:"write"}},{type:"content_block_delta",index:0,delta:{type:"input_json_delta",partial_json:"{"}},{type:"content_block_stop",index:0},{type:"message_delta",delta:{stop_reason:"tool_use"}},{type:"message_stop"}],
  ])("weist unvollständige/fehlerhafte Streams zurück: %j", async (...events) => {
    vi.stubEnv("ZAI_API_KEY", "test-key");
    vi.stubGlobal("fetch",vi.fn().mockResolvedValue(sseResponse(events.map(e=>JSON.stringify(e)))));
    await expect(collect([{role:"user",content:"test"}])).rejects.toThrow();
  });

  it("führt nur vollständig abgeschlossene Werkzeugblöcke aus", async () => {
    vi.stubEnv("ZAI_API_KEY", "test-key");
    vi.stubGlobal("fetch",vi.fn().mockResolvedValue(sseResponse([
      {type:"content_block_start",index:0,content_block:{type:"tool_use",id:"x",name:"read"}},
      {type:"content_block_delta",index:0,delta:{type:"input_json_delta",partial_json:'{"id":"a"}'}},
      {type:"content_block_stop",index:0},
      {type:"message_delta",delta:{stop_reason:"tool_use"}},
      {type:"message_stop"},
    ].map(e=>JSON.stringify(e)))));
    expect(await collect([{role:"user",content:"test"}])).toEqual([
      {type:"tool_calls",toolCalls:[{id:"x",type:"function",function:{name:"read",arguments:'{"id":"a"}'}}]}, {type:"done"}
    ]);
  });
