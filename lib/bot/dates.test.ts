...
describe("parseFuzzyDate", () => {
  it("erkennt ein gültiges ISO-Datum", () => {
    expect(parseFuzzyDate("2025-08-01", MITTWOCH)).toEqual({ iso: "2025-08-01" });
  });

  it("meldet ein unmögliches ISO-Datum als Hinweis, nicht als Absturz", () => {
  ...
  it("versteht 'heute' und 'übermorgen'", () => {
  ...
  it("versteht 'nächsten Montag' als den kommenden, nicht den heutigen", () => {
    // Mittwoch -> naechster Montag ist in 5 Tagen (21.7.)
    expect(parseFuzzyDate("nächsten Montag", MITTWOCH)).toEqual({ iso: "2025-07-21" });
  });

  it("versteht einen bloßen Wochentagsnamen als nächstes Vorkommen", () => {
  ...
  it("gibt bei Unsinn null plus Hinweis zurück, keinen Fehler", () => {
    const result = parseFuzzyDate("irgendwann nächstes Jahrzehnt", MITTWOCH);
    expect(result.iso).toBeNull();
    expect(result.hint).toBeTruthy();
  });
});
