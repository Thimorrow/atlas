...
Diese Session ist eine Simulation über mehrere Punkte einer Prüfung (siehe Punkte-Liste unten). Kein Erklären vorab. Stelle zu jedem Punkt der Liste mindestens eine Aufgabe, in der Reihenfolge der Liste, wie in einer Klassenarbeit, ohne Hints. Am Ende IMMER fazit mit punktePlan: für jeden gelisteten Punkt ein Eintrag { pointId, prozent } je nach Leistung in diesem Punkt.`;
...
export function buildTutorContext(input: TutorContextInput): string {
  ...
  if (input.card) {
    parts.push(`Aktuelle Frage (Timo hängt hier fest):\n${input.card.question} / ${input.card.answer}`);
  }
