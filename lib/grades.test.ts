...
describe("pointsToGradeLabel", () => {
  ...
  it("rundet Zwischenwerte auf die nächste Punktzahl", () => {
    expect(pointsToGradeLabel(11.4)).toBe("2"); // -> 11
    expect(pointsToGradeLabel(11.5)).toBe("2+"); // -> 12
  });

  it("kappt Werte außerhalb von 0 bis 15", () => {
...
describe("subjectAverage", () => {
  ...
  it("nur mündliche Noten: der mündliche Schnitt ist der Fachschnitt", () => {
  ...
  it("gewichtet mündlich zu schriftlich mit 50 zu 50", () => {
    const r = subjectAverage([g("oral", 14), g("written", 8)], 50);
    expect(r.average?.points).toBe(11);
    expect(r.average?.label).toBe("2");
  });

  it("gewichtet mündlich zu schriftlich mit 40 zu 60", () => {
  ...
  it("Gewichtung null zählt die Note nicht mit", () => {
  ...
  it("sind ALLE Gewichte null, gibt es für die Gruppe keinen Schnitt", () => {
  ...
  it("Gewicht null auf einer Seite lässt die andere Seite allein stehen", () => {
    const r = subjectAverage([g("oral", 15, 0), g("written", 6, 1)]);
    expect(r.average?.points).toBe(6);
  });

  it("oralWeight 0 blendet den mündlichen Teil aus, wenn beide Seiten da sind", () => {
...
describe("overallAverage", () => {
  it("ohne Fächer mit Noten gibt es keinen Gesamtschnitt", () => {
    expect(overallAverage([])).toBeNull();
    expect(overallAverage([null, null])).toBeNull();
  });

  it("mittelt die Fachschnitte, jedes Fach zählt einmal", () => {
...
describe("sortSubjectsByAverage", () => {
  ...
  it("sortiert nach Schnitt aufsteigend, das schwächste Fach zuerst", () => {
  ...
  it("trennt Fächer ohne Note in eine eigene Gruppe", () => {
...
describe("requiredPointsForGoal", () => {
  it("Normalfall: rechnet die nötige Punktzahl für die nächste schriftliche Note aus", () => {
  ...
  it("Ziel selbst mit 15 Punkten nicht erreichbar: sagt, was mit 15 herauskäme", () => {
  ...
  it("erste Note überhaupt in dieser Gruppe: die neue Note zählt allein", () => {
  ...
  it("die andere Gruppe hat keine Note: sie trägt nichts bei", () => {
  ...
  it("oralWeight 0: eine neue mündliche Note ändert am Schnitt nichts", () => {
    const r = requiredPointsForGoal(
  ...
  it("oralWeight 100: eine neue schriftliche Note ändert am Schnitt nichts", () => {
    const r = requiredPointsForGoal(
      [g("oral", 5), g("written", 8)],
      12,
      { kind: "written", weight: 1 },
  ...
  it("Gewichtung der neuen Note doppelt zählt stärker", () => {
  ...
  it("das Ergebnis liegt nie über 15 und ist immer eine ganze Zahl", () => {
