...
describe("renderMarkdown: Markdown-Syntax", () => {
  ...
  it("lässt mailto-Links zu", () => {
    expect(renderMarkdown("[Mail](mailto:a@b.de)")).toContain('<a href="mailto:a@b.de"');
  });

  it("gibt für leeren Body einen leeren String zurück", () => {
...
describe("renderMarkdown: Sicherheit", () => {
  ...
  it("lässt Ampersands als Text stehen, ohne HTML zu öffnen", () => {
...
describe("markdownPreview", () => {
  ...
  it("lässt keine verwaiste Klammer stehen, wenn das Linkziel selbst Klammern hat", () => {
...
describe("repairMissingParagraphBreaks", () => {
  ...
  it("trennt zwei direkt aneinandergrenzende Sätze auch ohne Liste", () => {
    expect(repairMissingParagraphBreaks("Das steht an.Morgen ist frei.")).toBe(
      "Das steht an.\n\nMorgen ist frei.",
    );
  });

  it("lässt normalen Text mit echten Wortzwischenräumen unverändert", () => {
  ...
  it("lässt bereits vorhandene Absätze unverändert", () => {
    const ok = "Erster Satz.\n\nZweiter Satz beginnt sauber.";
    expect(repairMissingParagraphBreaks(ok)).toBe(ok);
  });

  it("zerschneidet Eigennamen mit Großbuchstaben in der Mitte nicht", () => {
