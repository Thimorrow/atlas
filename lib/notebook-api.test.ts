...
vi.mock("@/lib/notebook-store", async (original) => ({ ...await original<typeof import("@/lib/notebook-store")>(), getNotebookPage: vi.fn(), updateNotebookPage: vi.fn(), notebookFilesBelongToSubject: vi.fn() }));
import { PATCH } from "@/app/api/notebooks/[id]/route";
import { getNotebookPage, notebookFilesBelongToSubject, updateNotebookPage } from "@/lib/notebook-store";
